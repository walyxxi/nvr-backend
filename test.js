...
const registerRecord = (data) => {
  return new Recorder({
    ...
    folder: `${__dirname}/media/${drive}`,
    name: data.name,
  });
};

let rec = registerRecord(cameraList[1]);
...
setInterval(() => {
  rec.stopRecording();
  fh.getDirectorySize(`${__dirname}/media/${drive}`, (err, size) => {
    ...
    if (size >= 3000000 * (80 / 100)) {
      ...
      rec = null;
      rec = registerRecord(cameraList[1], drive);
      rec.startRecording();
    } else {
      rec = null;
      rec = registerRecord(cameraList[1], drive);
  ...
  const path = `${__dirname}/media/${other_drive}/Gerbang Utama/`;
